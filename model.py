...
def run_news_model(username: str) -> dict:
    ...
    print(config.get("postgres", "host"), config.get("postgres", "user"), int(config.get("postgres", "port")), config.get("postgres", "password"))

    conn = psycopg2.connect(host=config.get("postgres", "host"),
